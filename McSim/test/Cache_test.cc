...
PthreadTimingSimulator* CacheTest::test_pts;
CacheL1* CacheTest::test_l1i;
CacheL1* CacheTest::test_l1d;
CacheL2* CacheTest::test_l2;
...
TEST_F(CacheTest, CheckBuild) {
  ...
  EXPECT_EQ(test_o3core->cachel1i, test_l1i);  // o3core -> l1i$
  EXPECT_EQ(test_o3core->cachel1d, test_l1d);  // o3core -> l1d$
  EXPECT_EQ(test_l1i->lsus[0], test_o3core);   // l1i$ -> o3core
  EXPECT_EQ(test_l1d->lsus[0], test_o3core);   // l1d$ -> o3core
  EXPECT_EQ(test_l1i->cachel2, test_l2);       // l1i$ -> l2$
  EXPECT_EQ(test_l1d->cachel2, test_l2);       // l1d$ -> l2$
  EXPECT_EQ(test_l2->cachel1i[0], test_l1i);   // l2$ -> l1i$
  EXPECT_EQ(test_l2->cachel1d[0], test_l1d);   // l2$ -> l1d$
...
TEST_F(CacheTest, IsEmptyInitially) {
  ...
  EXPECT_EQ((uint)0, test_l1i->num_rd_access + test_l1i->num_rd_miss + test_l1i->num_wr_access + test_l1i->num_wr_miss);
  EXPECT_EQ((uint)0, test_l1i->num_ev_coherency + test_l1i->num_ev_capacity + test_l1i->num_coherency_access);

  EXPECT_TRUE(test_l1d->req_event.empty());
  EXPECT_TRUE(test_l1d->rep_event.empty());
  EXPECT_EQ((uint)0, test_l1d->num_rd_access + test_l1d->num_rd_miss + test_l1d->num_wr_access + test_l1d->num_wr_miss);
  EXPECT_EQ((uint)0, test_l1d->num_ev_coherency + test_l1d->num_ev_capacity + test_l1d->num_coherency_access);
}

TEST_F(CacheTest, BasicCase) {
  uint64_t test_address = TEST_ADDR_D;
//  uint32_t set = (test_address >> test_l1d->set_lsb) % test_l1d->num_sets;
//  uint64_t tag = (test_address >> test_l1d->set_lsb) / test_l1d->num_sets;
//  uint32_t bank = (test_address >> test_l1d->set_lsb) % test_l1d->num_banks;
  ...
  EXPECT_EQ((unsigned int)0, test_l1d->num_rd_miss);
  ...
  EXPECT_EQ((unsigned int)1, test_l1d->num_rd_miss);
  EXPECT_EQ((unsigned int)1, test_l2->req_event.size());

  EXPECT_EQ((unsigned int)0, test_l2->num_rd_access);
  EXPECT_EQ((unsigned int)0, test_l2->num_rd_miss);

  curr_time += 40;  // l1_to_l2_t == 40
  test_l2->process_event(curr_time);
  
  EXPECT_EQ((unsigned int)1, test_l2->num_rd_access);
  EXPECT_EQ((unsigned int)1, test_l2->num_rd_miss);
...
//  EXPECT_EQ(tag, test_l1d->tags[set][1]->first);
//  std::cout << test_l1d->tags[set][0]->first << std::endl;
//  std::cout << test_l1d->tags[set][1]->first << std::endl;
//  std::cout << test_l1d->tags[set][2]->first << std::endl;
//  std::cout << test_l1d->tags[set][3]->first << std::endl;

//  std::cout << test_l1d->tags[set][0]->second << std::endl;
}
